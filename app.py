...
# ─── UI 레이아웃 ───
st.title("📊 퀀트 백테스팅 대시보드 (Streamlit)")
st.markdown("CORS 프록시 한계를 벗어난 100% 파이썬 클라우드. **한국 주식은 `FinanceDataReader`를 사용해 결측치 없이 완벽히 로드합니다.**")
...
col_krw, col_start, col_end, col_fx = st.columns(4)
krw = col_krw.number_input("총 투자금액 (원화)", min_value=1000000, value=100000000, step=1000000)
start_date = col_start.date_input("시작일", datetime(2015, 1, 1))
end_date = col_end.date_input("종료일", datetime.today())
fx = col_fx.number_input("환율 (KRW/USD)", value=1500, step=10)

strategy = st.radio("투자 방식", ["거치식 (첫날 전액 투자)", "적립식 (매일 분할 투자)"], horizontal=True)
        ...
        for i, raw in enumerate(raw_inputs):
            ...
            try:
                ...
                if ticker.isdigit() and len(ticker) == 6:
                    ...
                    source = "FDR (한국시장 전용)"
                # 2. 미국 주식 분기 처리 (Yahoo Finance 사용)
                else:
                ...
                if "거치식" in strategy:
                    ...
                    invested = pd.Series(total_usd, index=series.index)
                else:
                    daily_usd = total_usd / len(series)
                    shares = (daily_usd / series).cumsum()
                    portfolio = shares * series
                    invested = pd.Series([daily_usd * (k + 1) for k in range(len(series))], index=series.index)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    mode='lines',
                    name=f"{raw}  ${final_val:,.0f} ({final_roi:+.1f}%)",
                    hovertemplate="<b>%{x|%Y-%m-%d}</b><br>평가금: $%{y:,.2f}<br>수익률: %{customdata[0]:,.2f}%<br>투입원금: $%{customdata[1]:,.2f}<extra></extra>",
                ...
                summary_data.append({
                    ...
                    "시작가": f"${series.iloc[0]:.4f}",
                    "최종가": f"${series.iloc[-1]:.4f}",
                    "투입 원금": f"${final_inv:,.0f}",
                    "최종 평가금": f"${final_val:,.0f}",
    ...
    if has_data:
        if max_dates is not None:
            fig.add_trace(go.Scatter(
                ...
                hovertemplate='<b>%{x|%Y-%m-%d}</b><br>누적 원금: $%{y:,.2f}<extra></extra>',
